...
REPO_ROOT="${1:?usage: live_restart_validate_batch.sh <repo-root> [device] [volume-name]}"
DEVICE="${2:-/dev/disk12s1}"
EXPECTED_NAME="${3:-NTFS_STRESS}"
DEVICE_ID="${DEVICE##*/}"
...
if [[ "${1:-}" == "--format-first" ]]; then
  FORMAT_FIRST=1
  shift
  REPO_ROOT="${1:?usage: live_restart_validate_batch.sh [--format-first] <repo-root> [device] [volume-name]}"
  DEVICE="${2:-/dev/disk12s1}"
  EXPECTED_NAME="${3:-NTFS_STRESS}"
  DEVICE_ID="${DEVICE##*/}"
  VALIDATOR_SOURCE="$REPO_ROOT/scripts/live_ntfs_full_validation.sh"
...
echo "NTFS Access live restart+validate batch"
echo "repo=$REPO_ROOT"
echo "device=$DEVICE"
...
if [[ "$FORMAT_FIRST" -eq 1 ]]; then
  echo "--- Formatting sacrificial NTFS volume $DEVICE as $EXPECTED_NAME ---"
  run_with_timeout 20 /usr/sbin/diskutil listFilesystems >/dev/null 2>&1 || true
  /usr/sbin/diskutil eraseVolume "NTFS Access" "$EXPECTED_NAME" "$DEVICE"
  run_with_timeout 20 /usr/sbin/diskutil unmount force "$DEVICE" >/dev/null 2>&1 || true
fi
